...
def main():
    ...
    manipulatedVariables['deviationFor2DAction'] = [1.0]#[0.3, 1.0, 3.0, 9.0]
    ...
    for fileName in targetFileNameList:
        ...
        rawPickleList.extend(data)

    df = pd.DataFrame(rawPickleList)
    print(df)
    df2 = df['condition'].apply(pd.Series)
    allTrajectoryDf = pd.concat([df,df2],axis=1)
    ...
    def getTrajectories(df,dataFrame):
        ...
        for index,row in targetDf.iterrows():
            traj = row['trajectory']
            allTrajctery.extend([traj])
        # print(allTrajctery)
        return allTrajctery
    loadTrajectoriesFromDf = lambda df: getTrajectories(readParametersFromDf(df),allTrajectoryDf) 
