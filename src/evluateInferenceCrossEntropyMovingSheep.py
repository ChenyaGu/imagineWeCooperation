...
from src.inference.intention import CreateIntentionSpaceGivenSelfId, CalIntentionValueGivenState, \
    AdjustIntentionPriorGivenValueOfState, UpdateIntention
from src.inference.inference import CalUncommittedAgentsPolicyLikelihood, CalCommittedAgentsPolicyLikelihood, \
    InferOneStep
...
from src.sampleTrajectoryTools.trajectoriesSaveLoad import GetSavePath, readParametersFromDf, LoadTrajectories, \
    SaveAllTrajectories, \
...
class MeasureCrossEntropy:
    ...
    def __call__(self, trajectory):
        ...
        crossEntropies = [
            stats.entropy(baseDistribution) + np.mean([stats.entropy(baseDistribution, nonBaseDistribution)
...
class MeasureConvergeRate:
    ...
    def __call__(self, trajectory):
        ...
        intentionSamplesTrajectory = [[[self.chooseIntention(prior[agentId])[0] for agentId in self.imaginedWeIds]
                                      ...
                                      for prior in priors]
        convergeRates = [
            np.mean([np.all([intentionEachAgent == intentionSample[0] for intentionEachAgent in intentionSample])
...
class MeasureConvergeOnGoal:
    ...
    def __call__(self, trajectory):
        priors = [timeStep[self.priorIndex] for timeStep in trajectory]
        goalSamplesTrajectory = [
            [[self.chooseIntention(prior[agentId])[0] for agentId in range(len(self.imaginedWeIds))]
             for _ in range(30)]
            for prior in priors]
...
def main():
    ...
    getImaginedWeIdsForConvergeRate = lambda df: list(
        range(readParametersFromDf(df)['numSheep'], readParametersFromDf(df)['numSheep'] + numWolves))
    measureConvergeRate = lambda df: MeasureConvergeOnGoal(getImaginedWeIdsForConvergeRate(df), priorIndexinTimestep,
    ...
    for key, group in statisticsDf.groupby(['deviationFor2DAction', 'rationalityBetaInInference']):
        ...
        for numSheep, grp in group.groupby('numSheep'):
            df = pd.DataFrame(grp.values[0].tolist(), columns=list(range(maxRunningSteps)), index=['mean', 'se'])[
            ...
            dfTransPosed.plot.line(ax=axForDraw, label='numSheep = {}'.format(numSheep), y='mean', yerr='se',
                                   ylim=(0, 1.01), rot=0)
